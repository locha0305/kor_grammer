...
class Grammer():
    ...
    def Eval(self, statement):
        ...
        while cursor < len(statement):
            ...
            try:
                if letter in Operator.operator['one_char']:
                    if word.isdigit(): #만약 수라면
                        result.append(word)
                    else:
                        result.append(word)
                    if letter + statement[cursor + 1] in Operator.operator['two_char']:
                        result.append(Operator.operator['two_char'][letter + statement[cursor + 1]]) #연산자 추가
                        word = '' #단어 초기화
                        cursor += 1 #한번 건너뜀(2개라서)
                    else:
                        if letter != '$': #만약 마지막이 아나라면
                            result.append(Operator.operator['one_char'][letter]) #연산자 추가
                        word = '' #단어 초기화
                elif letter + statement[cursor + 1] in Operator.operator['two_char']:
                    if word.isdigit(): #만약 수라면
                        result.append(word)
                    else:
                        result.append(word)
            ...
            while cursor < len(line):
                ...
                if letter in identifier.identifier:
                    if letter == "는" or letter == "은": #변수값 설정문
                        ...
                        var_value = self.Eval(line_[1].split(" ")[1]) #변수 값
                        ...
                        if word == "동안": #while문
                            ...
                            self.result.append(keyword_tokens.TT_while_statement(self.Eval(line_[0].strip("인")), instant_runner.tokenize()))
                    elif letter == " ":
                        if word in grammer.grammer:
                            if word == "변수": #변수 선언문
                                ...
                                self.result.append(keyword_tokens.TT_if_statement(self.Eval(line_[1]), instant_runner.tokenize())) #if문안에 if문이 있을수 있어 토큰화 시켜 반환
                                word = '' #단어 초기화
